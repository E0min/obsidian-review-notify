...
- **배운 점**: SDK 없이 시그널링 채널을 설계하면서 SDP 오퍼·앤서와 ICE 후보 교환의 비동기 순서를 직접 제어했고, 시그널링 프로토콜과 미디어 전송 경로를 분리해 다루는 작업 흐름을 익혔습니다.

### Case 2. 혼잡도 데이터를 시그널링과 분리하기 위한 STOMP 채널 이중화
#### 1. 문제 원인
- 사일런트 컨퍼런스는 다수의 채널이 병렬로 진행되기 때문에 청중이 어떤 세션이 붐비는지 실시간으로 파악하지 못하면 채널 이동 의사결정을 내리기 어려웠고, 혼잡도 정보가 시그널링 흐름과 분리되어 있어야 운영 화면이 끊김 없이 갱신될 수 있었습니다.
...
- **배운 점**: 같은 STOMP 프로토콜 위에서도 도메인 단위로 채널을 분리해 장애 격리와 책임 분리를 동시에 확보했고, 동일한 reconnect·heartbeat 파라미터를 공유하면서 메시지 흐름만 독립적으로 두는 이중화 패턴을 코드로 정리했습니다.
...
- **배운 점**: 장애 회복 경로를 reconnectDelay 5초·heartbeat 4초 같은 운영 파라미터 수준에서 명시적으로 설계했고, 숫자 하나로 좀비 커넥션 감지 시점과 사용자 입장의 복구 속도를 조정해 두는 작업 방식을 익혔습니다.

### Case 4. SDP 협상 이전에 도착한 ICE 후보를 큐잉하여 연결 안정성 확보
#### 1. 문제 원인
...
#### 3. 결과
- **성과**: SDP와 ICE 도착 순서 역전으로 인한 초기 연결 실패가 사라졌고, 발표자·청중 양측 PeerConnection이 일관되게 ICE Connected 상태에 도달했습니다.
- **배운 점**: WebRTC의 비동기 시그널링에서는 상태 머신의 순서 의존성을 코드 레벨에서 보장해야 하며, 컴포넌트 ref에 둔 ICE 큐 버퍼링만으로도 초기 연결 실패 재현 사례를 제거했습니다.
